...
# if 'new_data.zip' not in os.listdir():
# os.system('wget https://cloud.tsinghua.edu.cn/f/07d6a0223d054247af26/?dl=1 -O new_data.zip')
...
with open('D:/Autoencoder/data/new_data/data/month_ret.pkl', 'rb') as f:
    print('Reading month_ret.pkl', end=' ')
    mon_ret = pd.read_pickle(f)
    mon_ret.to_pickle('data/month_ret.pkl')
    print('Done!')

with open('D:/Autoencoder/data/new_data/data/datashare.pkl', 'rb') as f:
...
def pre_process(date):
    ...
    re_df = pd.DataFrame(re_df, index=utils.CHARAS_LIST).T.fillna(0)
    re_df['permno'] = list(cross_slice['permno'].astype(int))
    re_df['DATE'] = list(cross_slice['DATE'].astype(int))

    return re_df[['permno', 'DATE'] + utils.CHARAS_LIST]
...
def cal_portfolio_ret(it, df):
    ...
    long_portfolio = \
        df.loc[df.DATE == d][['permno', f]].sort_values(by=f, ascending=False)[:df.loc[df.DATE == d].shape[0] // 10][
            'permno'].to_list()
    short_portfolio = \
        df.loc[df.DATE == d][['permno', f]].sort_values(by=f, ascending=False)[-df.loc[df.DATE == d].shape[0] // 10:][
            'permno'].to_list()
    # long-short portfolio return
    long_ret = mon_ret.loc[mon_ret.date == d].drop_duplicates('permno').set_index('permno').reindex(long_portfolio)[
        'ret-rf'].dropna().mean()
    short_ret = mon_ret.loc[mon_ret.date == d].drop_duplicates('permno').set_index('permno').reindex(short_portfolio)[
        'ret-rf'].dropna().mean()
...
def cal_portfolio_charas(month, df):
    mon_portfolio_chara = []
    p_name = ['p_' + chr for chr in utils.CHARAS_LIST]
    for chr in utils.CHARAS_LIST:
        long_portfolio = df.loc[df.DATE == month].sort_values(by=chr, ascending=False).reset_index(drop=True)[
                         :df.loc[df.DATE == month].shape[0] // 10]['permno'].to_list()
        short_portfolio = df.loc[df.DATE == month].sort_values(by=chr, ascending=False).reset_index(drop=True)[
                          -df.loc[df.DATE == month].shape[0] // 10:]['permno'].to_list()

        long_charas = df.loc[df.DATE == month].set_index('permno').loc[long_portfolio][utils.CHARAS_LIST]
        short_charas = df.loc[df.DATE == month].set_index('permno').loc[short_portfolio][utils.CHARAS_LIST]

        mon_portfolio_chara.append([month] + (0.5 * (long_charas.mean() - short_charas.mean())).to_list())

    return pd.DataFrame(mon_portfolio_chara, index=p_name, columns=['DATE'] + utils.CHARAS_LIST)


if __name__ == '__main__':
    # pre-process share data
    processed_df = Parallel(n_jobs=-1)(delayed(pre_process)(d) for d in
                                       tqdm(datashare.DATE.drop_duplicates().to_list(), colour='green',
                                            desc='Processing'))
